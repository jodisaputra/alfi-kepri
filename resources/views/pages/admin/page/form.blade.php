        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-10">
                    <x-card>
                        <x-card-header>
                            <x-link style="info btn-sm" url="{{ route('pages.index') }}"><i class="fas fa-arrow-left"></i>
                                Back</x-link>
                        </x-card-header>
                        <x-card-body>
                            <form action="{{ $action }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <x-label label="template">Template</x-label>
                                    <select class="form-control select2 @error('template') is-invalid @enderror"
                                        name="template">
                                        <option value="">Choose one</option>
                                        @foreach ($templatePages as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $template == $item->id ? 'selected' : null }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('template')
